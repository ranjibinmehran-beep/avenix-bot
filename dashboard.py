...
TXT = {
    "fa": {
        ...
        "tab_chart": "📊 چارت تریدینگ‌ویو",
        "tab_brain": "🧠 اتاق فرمان مغز ربات (AI)",
        "tab_signals": "📢 آرشیو سیگنال‌ها",
        "tab_contest": "🏆 اتاق اختصاصی فانددنکست (FundedNext Portal)",
        "tab_settings": "⚙️ تنظیمات سیستم",
...
if not st.session_state.terms_accepted:
    ...
    # ----------------- ACTIVE MAIN TABS (5-Tab Layout) -----------------
    tab_chart_view, tab_brain_view, tab_signals_view, tab_contest_view, tab_settings_view = st.tabs([
        t["tab_chart"], t["tab_brain"], t["tab_signals"], t["tab_contest"], t["tab_settings"]
    ])
    ...
    with tab_chart_view:
        ...
        # --- ⚡ HIGH-SPEED QUICK TRADING TERMINAL DIRECTLY OVERLAYING CHART (پنل معامله فوق سریع روی چارت) ---
        ...
        with col_exec_lot:
            # Sizing selectbox (enforce standard lot size)
        ...
        with col_exec_buy:
            if st.button("🚀 BUY (LONG) | خرید فوری", use_container_width=True):
                with st.spinner("Executing BUY..."):
                    # Calculate Stop Loss and TPs automatically based on SL percentage ratio!
                    ...
                    # Force override LOT size with input value
    ...
    # ----------------- 🏆 TAB 4: DEDICATED FUNDEDNEXT PORTAL (اتاق اختصاصی فانددنکست) -----------------
    ...
    with tab_settings_view:
        ...
        with st.expander("🏆 ۵. تنظیمات تهاجمی مسابقات و چالش‌های پروپ‌فرم (Contest Mode)"):
        ...
        with st.expander("✉️ ۶. اتاق مدیریت انتشار سیگنال‌ها (Bale, Telegram, WhatsApp)"):
        ...
        with st.expander("📊 ۷. جفت‌ارزهای فعال تحت نظر و تایم‌فریم اسکن"):
