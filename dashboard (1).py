...
st.markdown("<p style='text-align: center; color: #64748b; font-size: 14px; margin-bottom: 25px;'>پلتفرم معاملاتی هوشمند فارکس، فلزات گرانبها و ارزهای دیجیتال</p>", unsafe_allow_html=True)
...
with tab_chart:
    ...
    # This is 100% full screen on mobile, pinch-to-zoom, supports drawings and all indicators natively!
    tradingview_html = f"""
    ...
    st.markdown("<p style='color: #94a3b8; font-size: 12px;'>💡 چارت تعاملی تریدینگ‌ویو (کاملاً ریسپانسیو و تمام‌صفحه با قابلیت زوم، ترسیم خطوط و تغییر اندیکاتورها)</p>", unsafe_allow_html=True)
...
with tab_signals:
    ...
    st.markdown("<p style='color: #94a3b8; font-size: 13px;'>سیگنال‌های صادر شده با گزارش تفصیلی و علت ورود مغز ربات</p>", unsafe_allow_html=True)
